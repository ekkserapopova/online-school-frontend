...
const EditItemForm: React.FC<EditItemFormProps> = ({ type, item, onCancel, onSubmit }) => {
  ...
  const [end, setEnd] = useState(item.end || item.deadline || '');
  const [questionCount, setQuestionCount] = useState(item.count_questions?.toString() || '');
  const [timeLimit, setTimeLimit] = useState(item.time_limit?.toString() || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLesson) {
      ...
      updatedItem.deadline = end ? `${end}T23:59:59Z` : '';
    } else if (isTest) {
      updatedItem.deadline = end ? `${end}T23:59:59Z` : '';
    ...
    <div className={`edit-item edit-item--${type}`}>
      ...
      <form onSubmit={handleSubmit} className="edit-item__form">
        ...
        <div className="edit-item__actions">
          <button type="button" onClick={onCancel} className="edit-item__cancel-button">
            Отмена
          </button>
          <button type="submit" className="edit-item__submit-button" disabled={!name.trim()}>
            Сохранить
          </button>
        </div>
      </form>
